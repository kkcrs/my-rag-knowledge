...
export function DocumentDetailPage() {
  ...
  const onPickReindexFile = (e: React.ChangeEvent<HTMLInputElement>) => {
  ...
  return (
    <div>
        ...
        {isAdmin ? (
          <>
            <input
              ...
              style={{ display: 'none' }}
              onChange={onPickReindexFile as never}
            />
            <Button
      ...
      <Card title="切分结果" style={{ marginBottom: 24 }}>
        <ChunksSection
  ...
  function PreviewArea({ mimeType, previewUrl }: { mimeType: string; previewUrl: string }) {
    ...
    if (isMarkdownMime(mimeType)) {
      return <MarkdownPreview url={previewUrl} key={previewUrl} />
    }
