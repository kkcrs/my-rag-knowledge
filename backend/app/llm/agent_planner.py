...
class AgentDecision:
    ...
    # new_query / new_route 仅在对应 action 下有含义，否则为 None
    # ---
    action: AgentAction
    reason: str
...
class AgentPlanner:
    ...
    async def plan(
        ...
        history = format_history(previous_steps)
...
def format_history(steps: list[dict]) -> str:
    """把历史 agent_steps 压缩成一段可读文本给 LLM 看。"""
    if not steps:
        return "(无)"
    lines: list[str] = []
    for step in steps:
...
def parse_decision(raw: str) -> AgentDecision:
    """解析 LLM JSON 输出；任何异常都走 降级 proceed。"""
    # 容错：模型偶尔会包一层 ```json ...```
    text = raw.strip()
    if text.startswith("```"):
        text = text.split("\n", 1)[-1]
    ...
    # rewrite_query 必须带 new_query；没关键词就 proceed
...
def extract_text(content: str | list[str | dict]) -> str:
    """兼容 Langchain ChatModel 的 content 混合类型。"""
