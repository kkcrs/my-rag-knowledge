...
class RagasMetrics:
    """单条 case 的 RAGAS 4 指标结果，全部 [0, 1] U {None}。"""
...
async def evaluate_batch(samples: list[RagasSample]) -> list[RagasMetrics]:
    ...
    try:
        ...
        logger.exception("RAGAS evaluate 整体失败，返回 None 占位")
...
def _extract_metrics(result, expected: int) -> list[RagasMetrics]:
    ...
    if len(rows) != expected:
        logger.warning(
            "RAGAS 返回行数 %d 与样本数 %d 不一致，按可考值对齐", len(rows), expected
        )
...
def _pick(row: dict, key: str) -> float | None:
    """从 RAGAS 结果行里取分数；NaN / 缺失 / 转换错都视为 None。"""
    value = row.get(key)
    if value is None:
        return None
    try:
